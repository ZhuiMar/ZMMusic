...
class searchStoryVC: UIViewController {
...
// MARK: 设置通知
extension searchStoryVC {
    
    func setNotice() {
        NotificationCenter.default.addObserver(self, selector: #selector(searchStoryVC.loadData(_:)), name: Notification.Name(rawValue: "loadData"), object: nil)
    }
    ...
    func loadData(_ notification: NSNotification) {
        ...
        WLTMusicSearchTool.getSearchStoryData(1, row: model.music, search: searchStr, type: 20) { (pageModel, models, error) in
            self.storys = models
        }
    }
}
...
extension searchStoryVC: UITableViewDelegate,UITableViewDataSource {
